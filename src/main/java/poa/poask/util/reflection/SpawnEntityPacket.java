...
public class SpawnEntityPacket {
    ...
    @SneakyThrows
    public static Object spawnEntityPacket(Location location, EntityType entityType, int id, int data){
        ...
        return CommonClassMethodFields.spawnPacketConstructor.newInstance(id,
                UUID.randomUUID(),
                location.getX(),
                location.getY(),
                location.getZ(),
                location.getPitch(),
                location.getYaw(),
                Reflection.getField(letter, CommonClassMethodFields.entityTypeClass, null),
                data,
                CommonClassMethodFields.vec3Class.getDeclaredConstructor(double.class, double.class, double.class).newInstance(0, 0, 0),
                (double) location.getYaw());

    }

    public static Object spawnEntityPacket(Location location, EntityType entityType, int id){
        return spawnEntityPacket(location, entityType, id, 0);
    }
